...
// ==================== 长按3秒解锁【修复版：更灵敏+电脑端显示】====================
(function(){
    const btn = document.getElementById('toggleSearchBtn');
    const contentWrap = document.getElementById('contentContainer');
    ...
    // 初始只显示按钮（电脑端也显示）
    btn.style.display = 'inline-block'; // 强制显示
    ...
    btn.style.touchAction = 'none'; // 禁止浏览器默认触摸行为
    ...
    btn.addEventListener('touchmove', clearTimer); // 滑动取消长按
    btn.addEventListener('mouseup', clearTimer);
    btn.addEventListener('mouseleave', clearTimer);
    btn.addEventListener('mousemove', clearTimer);
